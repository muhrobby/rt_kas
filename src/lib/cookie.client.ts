...
export function setClientCookie(key: string, value: string, days = 7) {
  ...
  // biome-ignore lint/suspicious/noDocumentCookie: Client-side preference tracking via cookie is intended to avoid SSR flash
...
export function deleteClientCookie(key: string) {
  // biome-ignore lint/suspicious/noDocumentCookie: Required to clear persisted cookie preferences
